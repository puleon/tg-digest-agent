...
| D16 · Digest pairwise | 8 weekly pairs on the owner's profile, judged in both orders by Qwen3.6 and gpt-oss-120b. Round 1 (global views baseline): v1 5 : 1 under both judges — and the longer issue won every verdict, because the baseline starved the cinema slots (3–6 items vs 8). Round 2 (views per topic, equal sizes): **3 : 2 and 3 : 1, position flips 0.38 / 0.50, longer side still wins 9/9** — inconclusive at n = 8, and the judges' biases are the finding. The reader-level evidence is D11's AUC; the owner's own pairwise verdicts are the next row. | [d16-digest](docs/experiments/d16-digest/README.md) |
...
| digest: pairwise vs baseline; share of unsupported claims | ✅ two pairwise rounds, both judges; inconclusive at n = 8 with the biases quantified ([D16](docs/experiments/d16-digest/README.md)); unsupported 25 % measured / 6 % audited ([D15](docs/experiments/d15-faithfulness/README.md)) |
