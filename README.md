...
| D9 · Research tasks | 20 multi-step tasks with corpus-checked facts: **19/20** in every run, ~8 steps / 4 200–4 600 tokens per task. The external step reached a source in 0/20 (tunnel proxy down), then 9/20 (Wikipedia search on the whole request is empty for half, long articles exceed the page limit) — three bugs found by measuring, fixed one by one; ⏳ run D with the fixes. The corpus carried the facts on its own: the web step is verification, not retrieval. | [d9-agent](docs/experiments/d9-agent/README.md) |
